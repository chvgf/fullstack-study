  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        {/* URL 파라미터는주소의 경로에 유동적인 값을 넣는 형태고,
        쿼리스트링은 주소의 뒷부분에 ? 문자열 이후에 key=value 형태로 값을 정의하며 &로 구분을 하는 형태 */}

        {/* 주로 URL 파라미터는 ID 또는 이름을 사용하여 특정 데이터를 조회할 때 사용하고(네이버 뮤직 앨범각각 뒤에 숫자),
        쿼리스트링은 키워드 검색, 페이지네이션, 정렬 방식 등 데이터 조회에 필요한 옵션을 전달할 때 사용(네이버 검색했을때 주소 ? 뒤에)
