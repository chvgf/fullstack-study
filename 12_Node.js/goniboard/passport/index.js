...
module.exports = () => {
  ...
  passport.serializeUser((user, done) => {
    ...
    // done(null, { id:user_.id, username: user.username }); // (참고: 여러값 저장할때)
  });

  // 각 요청마다 실행되며, passport.sessiom 미들웨어가 이 메서드를 호출
  // serializeUser의 두 번째 인수로 넣었던 데이터가 deserializeUser의 매개변수가 됨
  ...
  passport.deserializeUser(async (id, done) => {
    ...
    try {
      ...
      // 이제 아무데서나 req.user을 쓰면 현재 로그인한 사용자의 정보를 쓸 수 있음
    } catch (err) {
      done(err)
    }
  });
